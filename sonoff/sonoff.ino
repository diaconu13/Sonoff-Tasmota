...
enum TasmotaCommands
{
  ...
  CMND_HUMIDITY_RESOLUTION,
  CMND_PRESSURE_RESOLUTION,
  CMND_POWER_RESOLUTION,
  CMND_VOLTAGE_RESOLUTION,
  CMND_CURRENT_RESOLUTION,
  CMND_ENERGY_RESOLUTION,
  CMND_MODULE,
  CMND_MODULES,
  CMND_GPIO,
  CMND_GPIOS,
...
char *GetOtaUrl(char *otaurl, size_t otaurl_size)
{
  ...
  { // OTA url contains placeholder for chip ID
...
void SetDevicePower(power_t rpower)
{
  ...
  { // Allow only one or no relay set
    power_t mask = 1;
    uint8_t count = 0;
    ...
    if (button_present)
    {
      if (SONOFF_4CHPRO == Settings.module)
      {
        ...
        if ((NOT_PRESSED == button) && (PRESSED == lastbutton[button_index]))
        {
          ...
          AddLog(LOG_LEVEL_DEBUG);
          if (!holdbutton[button_index])
            button_pressed = true; // Do not allow within 1 second
        }
        if (button_pressed)
        {
      ...
      else
      {
        ...
        else
        {
          ...
          { // Allow only single button press for immediate action
            ...
            {                                                                  // Button held for factor times longer
          ...
          else
          {
            if (!restart_flag && !holdbutton[button_index] && (multipress[button_index] > 0) && (multipress[button_index] < MAX_BUTTON_COMMANDS + 3))
            {
              ...
              else
              {
                ...
                { // Single or Double press
                  if (WifiState())
                  { // WPSconfig, Smartconfig or Wifimanager active
                    restart_flag = 1;
                  }
                  else
                  {
...
void ArduinoOTAInit()
{
  ...
  if (Settings.web_password[0] != 0)
    ArduinoOTA.setPassword(Settings.web_password);
  ...
  ArduinoOTA.onProgress([](unsigned int progress, unsigned int total) {
    if ((LOG_LEVEL_DEBUG <= seriallog_level))
    {
      ...
      Serial.printf(".");
      if (!(arduino_ota_progress_dot_count % 80))
        Serial.println();
    }
  });
  ...
  ArduinoOTA.onError([](ota_error_t error) {
    ...
    */
    ...
    if ((LOG_LEVEL_DEBUG <= seriallog_level) && arduino_ota_progress_dot_count)
      Serial.println();
    switch (error)
    {
    case OTA_BEGIN_ERROR:
      strncpy_P(error_str, PSTR(D_UPLOAD_ERR_2), sizeof(error_str));
      break;
    case OTA_RECEIVE_ERROR:
      strncpy_P(error_str, PSTR(D_UPLOAD_ERR_5), sizeof(error_str));
      break;
    case OTA_END_ERROR:
      strncpy_P(error_str, PSTR(D_UPLOAD_ERR_7), sizeof(error_str));
      break;
    default:
...
\*-------------------------------------------------------------------------------------------*/
    ...
    if (!Settings.flag.mqtt_serial)
    {
      if (isprint(serial_in_byte))
      {
        ...
        { // add char to string if it still fits
...
\*-------------------------------------------------------------------------------------------*/
    if (SONOFF_SC == Settings.module)
    {
      ...
      {                                               // Sonoff SC status from ATMEGA328P
...
void GpioInit()
{
  ...
  { // PWM Single color led (White)
    light_type = LT_PWM1;
  }
  else if (SONOFF_LED == Settings.module)
  { // PWM Dual color led (White warm and cold)
    light_type = LT_PWM2;
  }
...

{
  ...
  if (SONOFF_DUAL_R2 == Settings.module)
  {
    ...
    if (!openSent && !closeSent && !undefinedSent)
    {
      undefinedSent = true;
      Serial.printf("%s \n", "Gate UNDEFINED");

...
void MqttMyDataCb(const char *topic, uint32_t topic_len, const char *data, uint32_t data_len)
{
  ...
  if (0 == data_len)
    data = (const char *)&topic_copy + topic_len;
...
boolean MqttCheckTls()
{
  ...
  if (!result)
    AddLog_P(LOG_LEVEL_INFO, S_LOG_MQTT, PSTR(D_FAILED));
  EspClient.stop();
  yield();
  return result;
}
...
void MqttReconnect()
{
  ...
  { // Executed once just after power on and wifi is connected
#ifdef USE_MQTT_TLS
    if (!MqttCheckTls())
      return;
#endif // USE_MQTT_TLS
